...
🧠 개념 ————
[핵심 개념](#-핵심-개념) · [왜 FastAPI가 필요한가](#-왜-fastapi가-필요한가) · [베이스 이미지 선택](#-베이스-이미지-선택-slim-vs-alpine)
...
FastAPI는 Python 인터프리터 위에서 실행되는 웹 프레임워크입니다. 따라서 Docker 이미지에 Python 런타임이 포함된 베이스 이미지(`python:3.12-slim`)가 필요합니다. 이는 **런타임 환경의 일관성을 보장**하고, 어떤 호스트 OS에서도 동일하게 동작하게 하기 위함입니다.
