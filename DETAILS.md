...
**FastAPI기반 검색엔진 app (Elasticsearch + Kibana  Docker 최적화 가이드)**
...
| 대상 | 최적화 방식 | 이유 |
|------|------------|------|
| **FastAPI** | 멀티 스테이지 빌드, slim 베이스, non-root 실행 | Dockerfile을 작성하므로 완전한 제어 가능 |
| **Elasticsearch / Kibana** | JVM 튜닝, memory_lock, ulimits, healthcheck | Elastic사 완성 이미지 활용(내부 수정 불가, 설정 주입만 가능) |
...
FastAPI를 활용한 검색엔진app은 Python 인터프리터 위에서 프레임워크입니다. Python 인터프리터 위에서 실행되므로, Docker 이미지에 Python이 포함된 베이스 이미지(`python:3.12-slim`)가 필요합니다. //????
...
**이 프로젝트에서 slim을 런타임에도 쓰는 이유**: FastAPI(검색엔진app) + Elasticsearch는 확장 없는 순수 Python이라 alpine에서도 실행은 되지만, glibc 호환성 문제가 생길 수 있는 엣지케이스를 없애기 위해 slim을 선택했습니다.  //????

### slim base image custmizing
...
### Dockerfile을 작성 가능한 이미지 vs 불가능한 이미지

직접 Dockerfile을 작성할 수 있으면 경량화 가능, 완성된 이미지를 가져다 쓰면 설정 튜닝만 가능합니다.

| 이미지 | 경량화 적용 방식 | 경량화 적용 크기 |
|--------|------------|----------|
| FastAPI (검색엔진 app) | 멀티 스테이지 + slim | ~145MB |
...
## 🔧 Docker image build 및 repository 저장
